...
class RigidTransform(Transform3d):
    ...
    def __init__(
        self,
        R: Float[torch.Tensor, "..."],  # type hinting with no dimension specified
        ...
        # Note - if 3 and t are one batch, unsqueeze to add a batch dimm
        if R.dim() == 2 and t.dim() == 1:
            R = R.unsqueeze(0)
            t = t.unsqueeze(0)
        assert (batch_size := len(R)) == len(t), "R and t need same batch size"

        # Note - transformation matrix of batchx4x4
        ...
        matrix[..., 3, :3] = t
        # mat(3,3) = 1
        # mat(..., 3,:3) = 0 by default init
        matrix[..., 3, 3] = 1

        super().__init__(matrix=matrix, device=device, dtype=dtype)

    def get_rotation(self, parameterization=None, convention=None):
        # Note - retrieve the rotation 3x3 matrix and transpose it since it was R_transpose that was stored
    ...
    def inverse(self):
        ...
        # So this code inverses the transformation (rotation and translation) to recover the org point
        # p' = R*p + t
        # p = R_inv(p' - t)
    
        # Note - Swap the last 2 dim of 3x3 to get inverse/transpose
        
        R = self.get_rotation().transpose(-1, -2)
        t = self.get_translation()

        # Note -  Einstein summation - batch matrix-vector multiplication
        # bij (batch, row i, column j) bj (batch of translation vec) ->bi (result op - i is the resulting index after summation over j)
        # Essentially applies the rotation transformation to the translation vector. Not sure about flipping 
