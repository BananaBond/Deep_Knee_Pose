...
def trace_calls(frame, event, arg):
    '''
    utility function to trace calls inside a function when needed
    '''
    if event == 'call':
        code = frame.f_code
        func_name = code.co_name
        if not func_name.startswith("__") and not "__" in code.co_filename and not "site-packages" in code.co_filename:  # Exclude special functions
            print(f"Calling function: {func_name} in {code.co_filename}:{frame.f_lineno}")
    return trace_calls
